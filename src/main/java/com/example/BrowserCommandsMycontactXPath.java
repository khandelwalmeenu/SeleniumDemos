...
public class BrowserCommandsMycontactXPath {

	static WebDriver driver;
	static By UserName = By.xpath("//input[@id='user']");
	static By password = By.xpath("//input[@id='pass']");
	static By MarketingDepartment = By.xpath("//input[@value='0']");
	...
	static By dropdown =By.tagName("Select");
	...
	public static void main(String[] args) {
		...
		Sendtexttofield(UserName, "MeenaK");
		Sendtexttofield(password, "Test12345");

		clickmethod(MarketingDepartment);
		clickmethod(Sales);

		Sendtexttofield(subject, "Meenakshi Subject");
		Sendtexttofield(email, "qaz@yahoo.com");
		Sendtexttofield(q1, "abc");
		Sendtexttofield(q2, "Text MultiLine");
		...
		List<WebElement> allCheckBox = driver.findElements(checkBox);
		System.out.println("No of checkBox:" + allCheckBox.size());
		
		List<WebElement> allTextBox = driver.findElements(textBox);
		System.out.println("No of textBox:" + allTextBox.size());
		

		List<WebElement> alldropdown = driver.findElements(dropdown);
		System.out.println("No of dropdown:" + alldropdown.size());
		
		List<WebElement> allradiobutton = driver.findElements(radiobutton);
